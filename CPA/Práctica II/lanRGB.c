...
int resizeRGB(int w, int h, Byte *A, int w2, int h2, int la)
{
  ...
  double *aux2;
  double *aux3;
  aux1 = malloc(w * h2 * sizeof(double));
  aux2 = malloc(w * h2 * sizeof(double));
  aux3 = malloc(w * h2 * sizeof(double));
...
#pragma omp parallel sections
  {
#pragma omp section
    resize2D(w, h, A, w2, h2, aux1, la); /* red */
#pragma omp section
    resize2D(w, h, A + 1, w2, h2, aux2, la); /* green */
#pragma omp section
    resize2D(w, h, A + 2, w2, h2, aux3, la); /* blue */
  }
...
int main(int argc, char *argv[])
{
  ...
  {
    if (w2 == 0)
      w2 = round(w * (double)h2 / h);
    if (h2 == 0)
      h2 = round(h * (double)w2 / w);
  }
  else
  {
    w2 = round(w * e);
